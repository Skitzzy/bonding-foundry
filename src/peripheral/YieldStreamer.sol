...
import {IgTSRY} from "../interfaces/IgTSRY.sol";
...
/**
    @title YieldStreamer
    @notice This contract allows users to deposit their gTSRY and have their yield
...
contract YieldStreamer is IYieldStreamer, YieldSplitter {
    ...
    /**
        @notice Constructor
        @param gTSRY_ Address of gTSRY.
        @param sTSRY_ Address of sTSRY.
        @param TSRY_ Address of TSRY.
        @param streamToken_ Address of the token the TSRY will be swapped to.
        @param sushiRouter_ Address of sushiswap router.
        @param staking_ Address of sTSRY staking contract.
        @param authority_ Address of Olympus authority contract.
        @param maxSwapSlippagePercent_ Maximum acceptable slippage when swapping TSRY to streamTokens as percentage 6 decimals.
    ...
    */
    constructor(
        ...
        IERC20(gTSRY).approve(address(staking), type(uint256).max);
        IERC20(TSRY).approve(address(sushiRouter), type(uint256).max);
    }

    /**
        @notice Deposit gTSRY, creates a deposit in the active deposit pool to be unkept.
        @param amount_ Amount of gTSRY.
    ...
    */
    ...
    /**
        @notice Add more gTSRY to your principal deposit.
        @param id_ Id of the deposit.
        @param amount_ Amount of gTSRY to add.
    */
    ...
    /**
        ...
        @param id_ Id of the deposit.
        @param amount_ Amount of gTSRY to withdraw.
    */
    function withdrawPrincipal(uint256 id_, uint256 amount_) external override {
        ...
        if (amount_ < IgTSRY(gTSRY).balanceTo(depositInfo[id_].principalAmount)) {
            _withdrawPrincipal(id_, amount_, msg.sender);
            IERC20(gTSRY).safeTransfer(msg.sender, amount_);
        } else {
            ...
            (uint256 principal, uint256 totalGTSRY) = _closeDeposit(id_, msg.sender);
            ...
            IERC20(gTSRY).safeTransfer(msg.sender, principal);
            IERC20(gTSRY).safeTransfer(recipient, totalGTSRY - principal);
    ...
    /**
        @notice Withdraw excess yield from your deposit in gTSRY.
    ...
    */
    function withdrawYield(uint256 id_) external override {
        ...
        IERC20(gTSRY).safeTransfer(msg.sender, yield);
    }

    /**
        @notice Withdraw all excess yield from your all deposits you are the recipient of in gTSRY.
        @dev  Use withdrawYieldInStreamTokens() to withdraw yield in stream tokens.
    */
    function withdrawAllYield() external override {
        ...
        IERC20(gTSRY).safeTransfer(msg.sender, total);
    }
    ...
    */
    function redeemYieldOnBehalfOf(uint256 id_) external override returns (uint256 amount_) {
        ...
        IERC20(gTSRY).safeTransfer(recipientInfo[id_].recipientAddress, amount_);
    }
    ...
    */
    ...
    /**
        @notice Upkeeps all deposits if they are eligible.
                Upkeep consists of converting all eligible deposits yield from gTSRY into streamToken(usually DAI).
    ...
    */
    function upkeep() external override {
        ...
        for (uint256 i = 0; i < depositLength; i++) {
            ...
            if (_isUpkeepEligible(currentId)) {
                totalGTSRY += redeemableBalance(currentId);
            }
        }

        uint256 feeToDao = (totalGTSRY * feeToDaoPercent) / sixDecimalMaxNumber;
        IERC20(gTSRY).safeTransfer(authority.governor(), feeToDao);

        uint256 totalTSRYToSwap = staking.unstake(address(this), totalGTSRY - feeToDao, false, false);

        (, int256 TSRYOraclePrice, , , ) = priceConverterOracleWrapper.latestRoundData();
        uint256[] memory amounts = sushiRouter.swapExactTokensForTokens(
            totalTSRYToSwap,
            (uint256(TSRYOraclePrice) * (sixDecimalMaxNumber - maxSwapSlippagePercent)) / sixDecimalMaxNumber,
            sushiRouterPath,
            address(this),
            block.timestamp
        );
        ...
        for (uint256 i = 0; i < depositLength; i++) {
            ...
            if (_isUpkeepEligible(currentId)) {
                ...
                currentrecipientInfo.unclaimedStreamTokens += uint128(
                    (amounts[1] * _redeemYield(currentId)) / totalGTSRY
                );
    ...
    /**
        @notice Returns the total amount of yield in gTSRY the user can withdraw from all deposits. 
                Does not include harvestable stream tokens which is found in recipientInfo.
        @param recipient_ Address of recipient.
    */
    function totalRedeemableBalance(address recipient_) external view override returns (uint256 totalGTSRY) {
        for (uint256 i = 0; i < recipientIds[recipient_].length; i++) {
            totalGTSRY += redeemableBalance(recipientIds[recipient_][i]);
        }
    }

    /**
        @notice Gets the number of deposits eligible for upkeep and amount of TSRY of yield available to swap.
        ...
        @return amountOfYieldToSwap : total amount of yield in gTSRY ready to be swapped in next upkeep.
     */
    ...
    /**
        @notice Get deposits principal amount in gTSRY
        @param id_ Id of the deposit.
     */
    function getPrincipalInGTSRY(uint256 id_) external view returns (uint256) {
        return IgTSRY(gTSRY).balanceTo(depositInfo[id_].principalAmount);
    }
