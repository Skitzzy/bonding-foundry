...
contract sOlympus is IsTSRY, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgTSRY public gTSRY; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked TSRY", "sTSRY", 9) ERC20Permit("Staked TSRY") {
    ...
    function setgTSRY(address _gTSRY) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gTSRY) == address(0), "gTSRY:  gTSRY already set");
        require(_gTSRY != address(0), "gTSRY:  gTSRY is not a valid contract");
        gTSRY = IgTSRY(_gTSRY);
    }
    ...
    /**
        @notice increases rTSRY supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sTSRY of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sTSRY
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sTSRY: insufficient balance");
    }
    ...
    // toG converts an sTSRY balance to gTSRY terms. gTSRY is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gTSRY.balanceTo(amount);
    }

    // fromG converts a gTSRY balance to sTSRY terms. sTSRY is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gTSRY.balanceFrom(amount);
    }

    // Staking contract holds excess sTSRY
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gTSRY.balanceFrom(IERC20(address(gTSRY)).totalSupply())).add(
