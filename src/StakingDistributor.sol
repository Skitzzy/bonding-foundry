...
///         TSRY, as well as patches a small bug in the staking contract
...
contract Distributor is OlympusAccessControlled {
    ...
    /// The TSRY Token
    IERC20 private immutable TSRY;
    /// The Olympus Treasury
    ITreasury private immutable treasury;
    /// The TSRY Staking Contract
    ...
    function triggerRebase() external {
        unlockRebase = true;
        IStaking(staking).unstake(msg.sender, 0, true, true); // Give the caller the bounty TSRY.
    ...
    function nextRewardFor(address who) public view returns (uint256) {
        return (TSRY.balanceOf(who) * rewardRate) / DENOMINATOR;
    }
