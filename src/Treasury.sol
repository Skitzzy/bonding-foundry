...
contract OlympusTreasury is OlympusAccessControlled, ITreasury {
    ...
    /**
     * @notice allow approved address to deposit an asset for TSRY
    ...
    function deposit(
        ...
        // mint TSRY needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        TSRY.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn TSRY for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        TSRY.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new TSRY using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        TSRY.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or TSRY from the treasury, using sTSRY as collateral. This might allow an
     * sTSRY holder to provide TSRY liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(TSRY)) {
            require(permissions[STATUS.TSRYDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sTSRY.changeDebt(value, msg.sender, true);
        require(sTSRY.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(TSRY)) {
            TSRY.mint(msg.sender, value);
            TSRYDebt = TSRYDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sTSRY.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with TSRY
     * @param _amount uint256
     */
    function repayDebtWithTSRY(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.TSRYDEBTOR][msg.sender],
            notApproved
        );
        TSRY.burnFrom(msg.sender, _amount);
        sTSRY.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        TSRYDebt = TSRYDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(TSRY), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.STSRY) {
            sTSRY = IsTSRY(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.STSRY) {
            // 9
            sTSRY = IsTSRY(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(TSRY.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns TSRY valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(TSRY)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return TSRY.totalSupply() - TSRYDebt;
    }
}
