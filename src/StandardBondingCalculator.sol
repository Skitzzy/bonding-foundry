...
contract OlympusBondingCalculator is IBondingCalculator {
    ...
    IERC20 internal immutable TSRY;

    constructor(address _TSRY) {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(TSRY)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(TSRY), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(TSRY)).decimals())).div(getTotalValue(_pair));
    }
}
